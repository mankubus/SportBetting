...
public class SegmentedControlButton extends RadioButton {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isChecked()) {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xff7a7d84, 0xffb7babf });
            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
            textPaint.setColor(Color.WHITE);
        } else {
            GradientDrawable grad = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] { 0xffdcdde1, 0xffb4b9bf });
            grad.setBounds(0, 0, this.getWidth(), this.getHeight());
            grad.draw(canvas);
            textPaint.setColor(0xffcccccc);
            textPaint.setColor(R.color.llGreyColor);
        }
